...
// Function to extract key information from the CSV content
function extractFindingsData(csvContent) {
  ...
  for (let i = 1; i < lines.length; i++) { // Skip header
    ...
    // Look for patterns that indicate the start of a new finding
    if (line.includes('"C&P-') || line.includes('"IT-') || line.includes('"HR-') || line.includes('"FA-')) {
      ...
      // Continue reading until we find the next finding or end
      let j = i + 1;
      let detailLines = [];
      let responseLines = [];
      ...
      while (j < lines.length) {
        ...
        if (nextLine.includes('"C&P-') || nextLine.includes('"IT-') || nextLine.includes('"HR-') || nextLine.includes('"FA-')) {
          break;
        }
        
        // Look for specific patterns to extract other fields
        if (nextLine.includes('"2-Medium"') || nextLine.includes('"3-High"') || nextLine.includes('"1-Low"')) {
          // Extract rating information
          const ratingMatch = nextLine.match(/"([^"]+)"/);
          if (ratingMatch) {
            currentFinding.finding_rating = ratingMatch[1];
          }
          
          const controlMatch = nextLine.match(/"([^"]+)"/g);
          if (controlMatch && controlMatch.length > 1) {
        ...
        if (nextLine.includes('["') && nextLine.includes('"]')) {
          const recMatch = nextLine.match(/\["([^"]+)"\]/);
          if (recMatch) {
            currentFinding.recommendations = recMatch[1];
          }
        }
        ...
        if (nextLine.includes('Contracts & Procurement') || nextLine.includes('Information Technology') || 
          ...
          currentFinding.core_function = nextLine.replace(/"/g, '').trim();
        }
        
        // Look for created by
        if (nextLine.includes('Hamda Abdulla') || nextLine.includes('Other Name')) {
          currentFinding.created_by = nextLine.replace(/"/g, '').trim();
        }
...
// Function to recreate the table with a simpler schema
...
async function importExtractedData() {
  try {
    ...
    const [sampleData] = await connection.execute('SELECT id, sequence, year, audit_plan_name, finding_title FROM findings_csv_import LIMIT 10');
